...
export class AntColonyOptimizationAlgorithm {


    private graph: Graph = new Graph();


    private antList: Array<Ant> = [];
    ...
    public createGraph(): void {
        ...
        // set Ant image
        this.graph.putAllNodeProperty('image', antImage);
    ...
    private step() {
        ...
        this.graph.getAllEdges().forEach((edge: Edge) => {
            ...
            let pheromoneAmount: number = Math.max(this.graph.getProperty(edge, 'pheromone') * this.VOLATILIZATION_COEF, this.MIN_PHEROMONE_ON_EDGE);
    ...
    private displayCurrentShortest(answer: Set<number | string>) {
        ...
        answer.forEach(edgeId => {
            totalLength = totalLength + (this.graph.getEdge(edgeId) as any).length;
        });
...
class Ant {
    ...
    private choseNextEdgeByPheromone(): Edge {
        ...
        let chosenEdge: Edge | null = this.graph.choseProbabilisticallyEdgeFromAndExcept(
    ...
    private choseNextEdgeByRandom(): Edge {
        ...
        let chosenEdge: Edge | null = this.graph.choseRandomEdgeFromAndExcept(
            this.graph.getNeighborEdgesOf(this.currentNode.id!),
            [this.previousEdge.id!],
            this.random);
    ...
    private emitPheromone() {
        ...
        this.visitedEdgeIds.forEach(edgeId => {
            totalLength = totalLength + this.graph.getEdgeProperty(edgeId, 'length');
        });
        ...
        this.visitedEdgeIds.forEach(edgeId => {
            ...
            this.graph.putEdgeProperty(edgeId, 'pheromone',
                Math.min((this.graph.getEdgeProperty(edgeId, 'pheromone') + bonus), _this.MAX_PHEROMONE_ON_EDGE));
