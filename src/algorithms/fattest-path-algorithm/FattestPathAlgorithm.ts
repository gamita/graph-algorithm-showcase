...
export class FattestPathAlgorithm {
    ...
    public createGraph(): void {
        ...
        let random = new Chance('WidthSeed');
    ...
    private step() {
        ...
        if (this.START_NODE_ID !== maxFlowNode.id) this.graph.putNodeText(maxFlowNode.id!, this.graph.getProperty(maxFlowNode, 'flow').toFixed(0));
        ...
        connectedEdges.forEach((edge: Edge) => {
            ...
            let widthOnEdge: number = this.graph.getProperty(edge, 'width');
            let newCandidateFlow: number = Math.min(this.graph.getProperty(maxFlowNode, 'flow'), widthOnEdge);

            if (this.graph.getProperty(neighborNode, 'flow') < newCandidateFlow) {
    ...
    private getMaxFlowNode(): Node {
        ...
        let maxFlowNodeId = Array.from(this.flowUnfixedNodes).sort(function (nodeId1, nodeId2) {
            return _this.graph.getNodeProperty(nodeId2, 'flow') - _this.graph.getNodeProperty(nodeId1, 'flow');
        })[0];
