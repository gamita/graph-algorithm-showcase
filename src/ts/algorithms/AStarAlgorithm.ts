...
export default class AStartAlgorithm {
    ...
    private getMinDistanceNode(): Node {
        ...
        let minDistanceNodeId = Array.from(this.distanceUnfixedNodes).sort(function (nodeId1, nodeId2) {
            return (_this.graph.getNode(nodeId1)['distance'] + _this.calcHeuristicDistanceAt(nodeId1))

                - (_this.graph.getNode(nodeId2)['distance'] + _this.calcHeuristicDistanceAt(nodeId2));
        })[0];
