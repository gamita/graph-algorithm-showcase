...
export default class BidirectionalDijkstraAlgorithm {
    ...
    private stepFromStart() {
        this, this.step(this.NAME_SPACE_START);
    }



    private stepFromGoal() {
        this, this.step(this.NAME_SPACE_GOAL);
    }
    ...
    private step(nameSpace: string) {
        ...
        connectedEdges.forEach((edge: Edge) => {
            ...
            let currentDistance: number = neighborNode[nameSpace + 'distance'];
            let newCandidateDistance: number = minDistanceNode[nameSpace + 'distance'] + edge.length;
